...
import paralleltestingusingdataprovider.globalproperty.ExtentReportConfiguration;
...
public class Listeners  extends BaseTest implements ITestListener{
	...
	try {
		driver = (ThreadLocal<WebDriver>) result.getTestClass().getRealClass().getField("driver").get(result.getInstance());  //result has all information about the test that failed.So it can fetch the driver that is controlling the test method. This is the way to get the driver which is currently at the step where the test failed.
